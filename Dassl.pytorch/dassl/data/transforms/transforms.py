...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    if "random_resized_crop" in choices:
        scale = cfg.INPUT.get('CROP_SCALE', (0.08, 1.))
        print(f"+ random resized crop (size={cfg.INPUT.SIZE}) after scale={scale}")
        tfm_train += [
            RandomResizedCrop(cfg.INPUT.SIZE, scale=scale, interpolation=interp_mode)
        ]
